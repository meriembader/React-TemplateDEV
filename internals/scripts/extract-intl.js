 ...
 * and package them in the translation json files in the intl file.
 */
...
const { appLocales, DEFAULT_LOCALE } = require('../../app/www/intl/i18n');
...
// Store existing intl into memory
  ...
  try {
    ...
    for (const message of messages) {
      for (const locale of appLocales) {
        ...
        // Merge old intl into the babel extracted instances where react-intl is used
...
memoryTask.then(files => {
  ...
  extractTask.then(() => {
    ...
    mkdir('-p', 'app/intl'); // eslint-disable-line
